                  ...
                  const Real max_elem_size)
{
  ...
  for (auto i : make_range(n_points))
  {
    ...
    if (mid_points.size())
    {
      // When mid points are provided, max_elem_size is ignored and each segment has exactly one
      // edge, so the number of edges is 1.
      nums_edges_between_points.push_back(1);
      continue;
    }
...
buildPolyLineMesh(MeshBase & mesh,
                  ...
                  const Real max_elem_size)
{
  buildPolyLineMesh(mesh, points, {}, loop, start_boundary, end_boundary, max_elem_size);
}
...
namespace
{
// Returns the normal vector of the quadratic side `s` of `elem` at the local node index
// `key_node_index` on the reference side. Only valid for elements with EDGE3 sides.
Point
...
{
  // 1. Extract seed boundary polyline points (vertices + optional midpoints) from input_mesh.
  ...
  MooseMeshUtils::collectKeyPointsFromMesh(bdry_mh,
                                           cur_pts,
                                           cur_mids,
                                           /*skip_node_reduction=*/true);

  // 2. Geometric progression of incremental thicknesses. layer_thicknesses[i] is the offset
  ...
  // 3. Generate the N+1 polyline meshes. Polylines are indexed so that polyline 0 is geometrically
  ...
  for (auto layer_i : make_range(num_layers + 1))
  {
    ...
    auto ply = std::make_unique<libMesh::ReplicatedMesh>(mg.comm());
    ...
    if (layer_i + 1 < num_layers + 1)
    {
      ...
      auto ply_for_offset = std::make_unique<libMesh::ReplicatedMesh>(mg.comm());
      ...
      std::vector<Point> next_combined = MooseMeshUtils::generateOffsetPolyline(
  ...
  // 4. Triangulate each annulus (between polylines[i] and polylines[i+1]). Stitch each annulus to
  // the accumulating ring (except the first one).
  std::unique_ptr<MeshBase> ring;
  for (auto i : make_range(num_layers))
  {
    XYDelaunayOptions opts;
    opts.refine_bdy = false;
    opts.verify_holes = false;
    opts.stitch_holes = {i > 0};
    opts.refine_holes = {false};
    opts.tri_elem_type = std::string(tri_elem_type);
    if (output_subdomain_id != 0)
    {
      opts.has_output_subdomain_id = true;
      opts.output_subdomain_id = output_subdomain_id;
    }
    if (output_subdomain_name.size())
    {
      opts.has_output_subdomain_name = true;
      opts.output_subdomain_name = output_subdomain_name;
    }
    ...
    ring = MooseMeshUtils::triangulateWithDelaunay(
        mg, std::move(polylines[i + 1]), std::move(holes), opts);
  }
  return ring;
}
                        ...
                        const XYDelaunayOptions & opts)
{
  ...
  if (!opts.input_subdomain_names.empty())
  {
    ...
    const auto subdomain_ids = MooseMeshUtils::getSubdomainIDs(*mesh, opts.input_subdomain_names);
  ...
  poly2tri.set_interpolate_boundary_points(opts.add_nodes_per_boundary_segment);
  poly2tri.set_refine_boundary_allowed(opts.refine_bdy);
  poly2tri.set_verify_hole_boundaries(opts.verify_holes);

  poly2tri.desired_area() = opts.desired_area;
  poly2tri.minimum_angle() = 0; // Not yet supported
  poly2tri.smooth_after_generating() = opts.smooth_tri;
  ...
  for (auto hole_i : index_range(hole_meshes))
  {
    ...
    if (hole_i < opts.hole_boundary_id_filters.size() &&
        !opts.hole_boundary_id_filters[hole_i].empty())
      meshed_holes.emplace_back(*hole_meshes[hole_i], opts.hole_boundary_id_filters[hole_i]);
    else
    ...
    stitch_second_order_holes =
        opts.stitch_holes.empty() ? false
                                  : ((holes_with_midpoints[hole_i] && opts.stitch_holes[hole_i]) ||
                                     stitch_second_order_holes);
    if (hole_i < opts.refine_holes.size())
      meshed_holes.back().set_refine_boundary_allowed(opts.refine_holes[hole_i]);

    triangulator_hole_ptrs[hole_i] = &meshed_holes.back();
  }
  if (stitch_second_order_holes &&
      (opts.tri_elem_type == "TRI3" || opts.tri_elem_type == "DEFAULT"))
    mg.paramError(
        "tri_element_type",
  ...
  if (opts.desired_area_func != "")
  {
    // poly2tri will clone this so it's fine going out of scope
    libMesh::ParsedFunction<Real> area_func{opts.desired_area_func};
    poly2tri.set_desired_area_function(&area_func);
  }
  else if (opts.use_auto_area_func)
  {
    poly2tri.set_auto_area_function(
        mg.comm(),
        opts.auto_area_function_num_points,
        opts.auto_area_function_power,
        opts.auto_area_func_default_size > 0.0 ? opts.auto_area_func_default_size : 0.0,
        opts.auto_area_func_default_size_dist > 0.0 ? opts.auto_area_func_default_size_dist : -1.0);
  }
  ...
  // will be meshed.
  for (const auto & point : opts.interior_points)
  ...
  for (auto hole_i : index_range(hole_meshes))
  {
    if (hole_i < opts.stitch_holes.size() && opts.stitch_holes[hole_i])
    {
      ...
      // hole-with-holes.
      libMesh::TriangulatorInterface::MeshedHole mh{
          hole_mesh,
          (hole_i < opts.hole_boundary_id_filters.size() &&
           !opts.hole_boundary_id_filters[hole_i].empty())
                       ...
                       const Real thickness)
{
  ...
  if (points.empty())
  {
    ...
    MooseMeshUtils::collectKeyPointsFromMesh(bdry_mh, points, mid_points);
  }
