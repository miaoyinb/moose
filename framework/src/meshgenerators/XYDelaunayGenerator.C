...
{
  MooseMeshUtils::XYDelaunayOptions opts;
  ...
  opts.interior_points = _interior_points;
  opts.tri_elem_type = std::string(_tri_elem_type);
  opts.stitch_holes = _stitch_holes;
  opts.refine_holes = _refine_holes;
  opts.use_binary_search = (_algorithm == "BINARY");
  opts.verbose_stitching = _verbose_stitching;
  if (isParamValid("output_subdomain_id"))
  {
    opts.has_output_subdomain_id = true;
    opts.output_subdomain_id = getParam<SubdomainID>("output_subdomain_id");
  }
  ...
  const bool user_has_output_boundary = opts.has_output_boundary;
  const BoundaryName user_output_boundary = opts.output_boundary;
  ...
  const SubdomainID layer_sd_id =
      opts.has_output_subdomain_id ? opts.output_subdomain_id : SubdomainID(0);
  const SubdomainName layer_sd_name =
      opts.has_output_subdomain_name ? opts.output_subdomain_name : SubdomainName();
  ...
  if (using_outer_layer)
  {
    ...
    opts.input_boundary_names = {BoundaryName("1")};
    opts.has_output_boundary = true;
    opts.output_boundary = tmp_outer_name;
  }
  ...
  for (auto hole_i : index_range(_hole_ptrs))
  {
    if (hole_i < _holes_boundary_layer_num.size() && _holes_boundary_layer_num[hole_i] > 0)
    {
      ...
      if (keep_input)
      {
        ...
        auto & ring_u = dynamic_cast<libMesh::UnstructuredMesh &>(*hole_ring);
        auto & inp_u = dynamic_cast<libMesh::UnstructuredMesh &>(*hole_meshes[hole_i]);
        ...
        {
          const auto max_bid =
              std::max(*ring_bids.rbegin(), inp_bids.empty() ? boundary_id_type(0) : *inp_bids.rbegin());
          BoundaryID idx = 1;
          for (auto b : inp_bids)
          {
      ...
      if (opts.stitch_holes.size() <= hole_i)
        opts.stitch_holes.resize(_hole_ptrs.size(), false);
      opts.stitch_holes[hole_i] = true;
      ...
      if (opts.hole_boundary_id_filters.size() <= hole_i)
        opts.hole_boundary_id_filters.resize(_hole_ptrs.size());
  ...
  auto result = MooseMeshUtils::triangulateWithDelaunay(
      *this, std::move(boundary_mesh), std::move(hole_meshes), opts);
  ...
  if (outer_ring_clone)
  {
    ...
    auto & result_u = dynamic_cast<libMesh::UnstructuredMesh &>(*result);
    auto & clone_u = dynamic_cast<libMesh::UnstructuredMesh &>(*outer_ring_clone);
    ...
    libMesh::MeshTools::Modification::change_boundary_id(
        *result, ring_outermost_temp, sentinel_id);

    if (user_has_output_boundary)
    {
      auto user_id =
          MooseMeshUtils::getBoundaryIDs(*result, {user_output_boundary}, true).front();
