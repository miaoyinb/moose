...
BoundaryLayerTriangleGenerator::generate()
{
  ...
  if (_keep_input)
  {
    ...
    {
      const auto max_bid =
          std::max(ring_bids.empty() ? boundary_id_type(0) : *ring_bids.rbegin(),
  ...
  // Delete all intermediate ring bcids; keep only the interface/surface if requested.
  std::vector<BoundaryID> bids_to_delete;
  for (boundary_id_type b = 0; b < boundary_id_type(2 * _num_layers); ++b)
  {
    if (!_interface_name.empty() && b == interface_bid)
      continue;
    if (!_surface_name.empty() && b == surface_bid)
      continue;
