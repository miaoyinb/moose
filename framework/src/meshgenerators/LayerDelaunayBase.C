    ...
    const MeshGeneratorName & name_suffix)
{
  ...
  for (const auto & layer_i : make_range(layerthickness_values.size()))
  {
    ...
    auto params = _app.getFactory().getValidParams("PolyLineMeshParallelTo1DSidesetGenerator");
    ...
    addMeshSubgenerator("PolyLineMeshParallelTo1DSidesetGenerator", submg_name, params);
  }
