...
#include "PolyLineMeshParallelTo1DSidesetGenerator.h"
...
registerMooseObject("MooseApp", PolyLineMeshParallelTo1DSidesetGenerator);
registerMooseObjectRenamed("MooseApp",
                           GapLineMeshGenerator,
                           "06/30/2027 24:00",
                           PolyLineMeshParallelTo1DSidesetGenerator);

InputParameters
PolyLineMeshParallelTo1DSidesetGenerator::validParams()
{
  ...
  params.addParam<bool>(
      "skip_node_reduction",
      false,
      "Whether to skip the node reduction step, which discards the vertex nodes "
      "that are colinear with its two neighboring vertex nodes when generating "
      "the polyline mesh. Note that if this parameter is set to false, only vertex nodes of the "
      "polygon shape will be taken into account during mesh generation, which would also lose the "
      "midpoint information if the input mesh contains quadratic elements.");
...
PolyLineMeshParallelTo1DSidesetGenerator::PolyLineMeshParallelTo1DSidesetGenerator(
...
PolyLineMeshParallelTo1DSidesetGenerator::generate()
{
  ...
  MooseMeshUtils::collectKeyPointsFromMesh(
      bdry_mh, reduced_pts_list, reduced_mid_pts_list, _skip_node_reduction);
