 ...
 * @param points The list of points defining the polyline
 * @param mid_points The optional list of midpoints corresponding to the segments for quadratic
 * elements. If provided, max_elem_size is ignored and each pair of neighboring points must have
...
struct XYDelaunayOptions
{
  ...
  // Optional per-hole filter: when a hole mesh has multiple external manifolds (e.g. a layered
  // annular ring), MeshedHole cannot auto-pick one. Provide the boundary-id set that identifies
  // the desired outer manifold. Empty set for a given hole means auto-detect.
  std::vector<std::set<std::size_t>> hole_boundary_id_filters;
};

/**
 * Runs a 2D Delaunay triangulation (via libMesh::Poly2TriTriangulator) inside a closed boundary
 ...
 * @param boundary_mesh The closed-loop boundary mesh; ownership is transferred
 * @param hole_meshes One mesh per hole; ownership of each is transferred
 * @param opts Triangulation options (see XYDelaunayOptions)
 * @return The triangulated mesh with stitched holes
 */
 ...
 * either the outward or inward direction. Triangulates the input polyline mesh to recover
 * per-side normals, averages adjacent normals at each vertex to compute the offset direction,
 * applies a kite-geometry scaling so the resulting offset wall is at the requested thickness,
 * and verifies no self-intersection of the offset polyline. Errors via mg->paramError("thickness",
 * ...) if the offset would tangle the polyline.
 ...
 * @param mid_points The midpoints of the original polyline (for quadratic Edge3 input). If both
 *  this and points are empty, populated from the input mesh.
