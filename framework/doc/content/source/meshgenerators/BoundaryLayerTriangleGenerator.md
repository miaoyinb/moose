...
The `BoundaryLayerTriangleGenerator` creates a layered 2D triangulated mesh utilizing the [`XYDelaunayGenerator`](/XYDelaunayGenerator.md) based on a given surface of a 2D-XY mesh. Aside from `XYDelaunayGenerator`, this mesh generator uses a series of existing MOOSE mesh generator as its sub mesh generators to achieve the meshing tasks, including [`PolyLineMeshParallelTo1DSidesetGenerator`](/PolyLineMeshParallelTo1DSidesetGenerator.md) and [`StitchMeshGenerator`](/StitchMeshGenerator.md).

The `BoundaryLayerTriangleGenerator` creates a layered 2D triangle mesh along the specified 1D surface of a 2D-XY mesh. It uses several sub-generators:
- [`PolyLineMeshParallelTo1DSidesetGenerator`](/PolyLineMeshParallelTo1DSidesetGenerator.md) to generate the polyline mesh that defines the layer boundaries of the 2D triangulated mesh to be generated, which need to be parallel to the specified 1D surface of the input mesh.
- [`XYDelaunayGenerator`](/XYDelaunayGenerator.md) to generate the 2D triangulated meshes of layers. The generation of each layer uses the two neighboring polyline meshes as the input boundary and hole meshes, respectively.
- [`StitchMeshGenerator`](/StitchMeshGenerator.md) to stitch the generated layered 2D triangulated mesh together as well as to stitch the layered 2D triangulated mesh with the input mesh if applicable.
